...
#include <franka_example_controllers/cartesian_impedance_dead_zone_cbf_interaction_controller.h>
...
bool CartesianImpedanceDZCBFInteractionController::init(hardware_interface::RobotHW* robot_hw,
                                               ros::NodeHandle& node_handle) {
  std::vector<double> cartesian_stiffness_vector;
  std::vector<double> cartesian_damping_vector;

  sub_equilibrium_pose_ = node_handle.subscribe(
      "/trajectory_publisher/equilibrium_pose", 20, &CartesianImpedanceDZCBFInteractionController::equilibriumPoseCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());

  std::string arm_id;
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR_STREAM("CartesianImpedanceDZCBFInteractionController: Could not read parameter arm_id");
    return false;
  }

  if (!node_handle.getParam("/alpha", alpha)) {
    ROS_ERROR_STREAM("CartesianImpedanceDZCBFInteractionController: Could not read parameter alpha");
    return false;
  }

  if (!node_handle.getParam("/cbf_active", cbf_active)) {
    ROS_ERROR_STREAM("CartesianImpedanceDZCBFInteractionController: Could not read parameter cbf_active");
    return false;
  }

  if (!node_handle.getParam("/damping_ratio", damping_ratio)) {
    ROS_ERROR_STREAM("CartesianImpedanceDZCBFInteractionController: Could not read parameter damping_ratio");
    return false;
  }

  if (!node_handle.getParam("/Kmax", Kmax)) {
    ROS_ERROR_STREAM("CartesianImpedanceDZCBFInteractionController: Could not read parameter Kmax");
    return false;
  }
  ...
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "CartesianImpedanceDZCBFInteractionController: Invalid or no joint_names parameters provided, "
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceDZCBFInteractionController: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "CartesianImpedanceDZCBFInteractionController: Exception getting model handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceDZCBFInteractionController: Error getting state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "CartesianImpedanceDZCBFInteractionController: Exception getting state handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceDZCBFInteractionController: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "CartesianImpedanceDZCBFInteractionController: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  ...
  dynamic_server_compliance_param_->setCallback(
      boost::bind(&CartesianImpedanceDZCBFInteractionController::complianceParamCallback, this, _1, _2));
...
void CartesianImpedanceDZCBFInteractionController::starting(const ros::Time& /*time*/) {
...
void CartesianImpedanceDZCBFInteractionController::update(const ros::Time& time,
...
Eigen::Matrix<double, 7, 1> CartesianImpedanceDZCBFInteractionController::saturateTorqueRate(
...
Eigen::Matrix<double, 7, 1> CartesianImpedanceDZCBFInteractionController::saturateQdotRate(
...
std::tuple<Eigen::Matrix<double, 7, 1>, double> CartesianImpedanceDZCBFInteractionController::cbfCompute(
    const Eigen::Matrix<double, 7, 1>& tau_d_calculated,
...
std::tuple<Eigen::Matrix<double, 7, 1>, double> CartesianImpedanceDZCBFInteractionController::cbfAnalytical(
    const Eigen::Matrix<double, 7, 1>& tau_d_calculated,
    const Eigen::Map<Eigen::Matrix<double, 7, 7>>& D,
    const Eigen::Map<Eigen::Matrix<double, 7, 1>>& G,
    const Eigen::Map<Eigen::Matrix<double, 7, 1>>& dq) {
...
void CartesianImpedanceDZCBFInteractionController::complianceParamCallback(
...
void CartesianImpedanceDZCBFInteractionController::equilibriumPoseCallback(
...
Eigen::Matrix<double, 7, 1> CartesianImpedanceDZCBFInteractionController::EMA(
...
PLUGINLIB_EXPORT_CLASS(franka_example_controllers::CartesianImpedanceDZCBFInteractionController,
