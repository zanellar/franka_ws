...
#include <franka_example_controllers/cartesian_impedance_cbf_interaction_power_controller.h>
...
bool CartesianImpedanceCBFInteractionPowerController::init(hardware_interface::RobotHW* robot_hw,
                                               ros::NodeHandle& node_handle) {
  std::vector<double> cartesian_stiffness_vector;
  std::vector<double> cartesian_damping_vector;

  sub_equilibrium_pose_ = node_handle.subscribe(
      "/trajectory_publisher/equilibrium_pose", 20, &CartesianImpedanceCBFInteractionPowerController::equilibriumPoseCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());

  std::string arm_id;
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR_STREAM("CartesianImpedanceCBFInteractionPowerController: Could not read parameter arm_id");
    return false;
  }

  if (!node_handle.getParam("/alpha", alpha)) {
    ROS_ERROR_STREAM("CartesianImpedanceCBFInteractionPowerController: Could not read parameter alpha");
    return false;
  }

  if (!node_handle.getParam("/cbf_active", cbf_active)) {
    ROS_ERROR_STREAM("CartesianImpedanceCBFInteractionPowerController: Could not read parameter cbf_active");
    return false;
  }

  if (!node_handle.getParam("/damping_ratio", damping_ratio)) {
    ROS_ERROR_STREAM("CartesianImpedanceCBFInteractionPowerController: Could not read parameter damping_ratio");
    return false;
  }

  if (!node_handle.getParam("/Kmax", Kmax)) {
    ROS_ERROR_STREAM("CartesianImpedanceCBFInteractionPowerController: Could not read parameter Kmax");
    return false;
  }

  if (!node_handle.getParam("/Pmax", Pmax)) {
    ROS_ERROR_STREAM("CartesianImpedanceCBFInteractionPowerController: Could not read parameter Pmax");
    return false;
  }
  ...
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "CartesianImpedanceCBFInteractionPowerController: Invalid or no joint_names parameters provided, "
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceCBFInteractionPowerController: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "CartesianImpedanceCBFInteractionPowerController: Exception getting model handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceCBFInteractionPowerController: Error getting state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "CartesianImpedanceCBFInteractionPowerController: Exception getting state handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceCBFInteractionPowerController: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "CartesianImpedanceCBFInteractionPowerController: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  ...
  dynamic_server_compliance_param_->setCallback(
      boost::bind(&CartesianImpedanceCBFInteractionPowerController::complianceParamCallback, this, _1, _2));
...
void CartesianImpedanceCBFInteractionPowerController::starting(const ros::Time& /*time*/) {
...
void CartesianImpedanceCBFInteractionPowerController::update(const ros::Time& time,
  ...
  // dq_filtered = EMA(dq_saturated, dq_filtered, EMAlpha);

  std::tie(tau_cbf, hK) = cbfAnalytical(tau_d, mass, gravity, dq);

  // std::tie(tau_cbf, h) = cbfAnalytical(tau_d, mass, gravity, Eigen::Map<Eigen::Matrix<double, 7, 1>>(dq_saturated.data()));
  // remove gravity contribution from external torque
  Eigen::Matrix<double, 7, 1> tau_ext_flipped = -tau_ext + gravity;

  std::tie(tau_cbf, hK, hP) = cbfCompute(tau_d, mass, gravity, coriolis, Eigen::Map<Eigen::Matrix<double, 7, 1>>(dq_saturated.data()),tau_ext_flipped);
  ...
  // Saturate torque rate to avoid discontinuities -> handled by franka, this implementation does not work at all
...
Eigen::Matrix<double, 7, 1> CartesianImpedanceCBFInteractionPowerController::saturateTorqueRate(
...
Eigen::Matrix<double, 7, 1> CartesianImpedanceCBFInteractionPowerController::saturateQdotRate(
...
std::tuple<Eigen::Matrix<double, 7, 1>, double, double> CartesianImpedanceCBFInteractionPowerController::cbfCompute(
    const Eigen::Matrix<double, 7, 1>& tau_d_calculated,
    const Eigen::Map<Eigen::Matrix<double, 7, 7>>& D,
    const Eigen::Map<Eigen::Matrix<double, 7, 1>>& G,
    const Eigen::Map<Eigen::Matrix<double, 7, 1>>& C,
    const Eigen::Map<Eigen::Matrix<double, 7, 1>>& dq,
    const Eigen::Matrix<double, 7, 1>& tau_ext) {
  ...
  Eigen::Matrix<double, 7, 7> Dinv = D.inverse();
  // compute cbf value
  hK(0,0) = Kmax -.5 * dq.transpose() * D * dq;
  hP(0,0) = Pmax + tau_ext.transpose()*dq;

  // ROS_INFO("hP: %f",hP(0,0));
  ...
  constraint_matrix << dq.transpose(), -tau_ext.transpose() * Dinv, Eigen::Matrix<double, 7,7>::Identity(7,7);
  QPinstance.constraint_matrix = constraint_matrix.sparseView(); 

  // lower bound: not used, so set to -infinity
  lower_bounds << -kInfinity, -kInfinity, -tau_max_;
  QPinstance.lower_bounds = lower_bounds.sparseView();

  // upper bound: h' = alpha(Kmax - .5 * dq.T @ D @ dq) + G.T @ dq
  // hp = (alpha*h).sparseView();
  upper_bounds << alpha*hK - tau_ext.transpose()*dq, tau_ext.transpose()*Dinv*(tau_ext-C)+alpha*hP, tau_max_;
  // for (int i = 0; i < 8; i++) {
  ...
  double right = -alpha*hK[0,0];
  if(left-right < -abs(left)/100000){
    ROS_WARN("condition not met: %f - %f = %f", left, right, left-right);
  }

  return std::make_tuple(tau_d_new, hK(0,0), hP(0,0));
}

std::tuple<Eigen::Matrix<double, 7, 1>, double> CartesianImpedanceCBFInteractionPowerController::cbfAnalytical(
    const Eigen::Matrix<double, 7, 1>& tau_d_calculated,
    const Eigen::Map<Eigen::Matrix<double, 7, 7>>& D,
    const Eigen::Map<Eigen::Matrix<double, 7, 1>>& G,
    const Eigen::Map<Eigen::Matrix<double, 7, 1>>& dq) {
  ...
  double Psi = (-dq.transpose() * tau_d_calculated + alpha*hK)(0,0);
  if(Psi < 0){
  ...
  return std::make_tuple(tau_d_new, hK(0,0));
}

void CartesianImpedanceCBFInteractionPowerController::complianceParamCallback(
...
void CartesianImpedanceCBFInteractionPowerController::equilibriumPoseCallback(
...
Eigen::Matrix<double, 7, 1> CartesianImpedanceCBFInteractionPowerController::EMA(
...
PLUGINLIB_EXPORT_CLASS(franka_example_controllers::CartesianImpedanceCBFInteractionPowerController,
