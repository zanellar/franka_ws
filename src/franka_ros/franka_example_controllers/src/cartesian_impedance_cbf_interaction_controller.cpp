...
#include <franka_example_controllers/cartesian_impedance_cbf_interaction_controller.h>
...
bool CartesianImpedanceCBFInteractionController::init(hardware_interface::RobotHW* robot_hw,
                                               ros::NodeHandle& node_handle) {
  std::vector<double> cartesian_stiffness_vector;
  std::vector<double> cartesian_damping_vector;

  sub_equilibrium_pose_ = node_handle.subscribe(
      "/trajectory_publisher/equilibrium_pose", 20, &CartesianImpedanceCBFInteractionController::equilibriumPoseCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());

  std::string arm_id;
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR_STREAM("CartesianImpedanceCBFInteractionController: Could not read parameter arm_id");
    return false;
  }

  if (!node_handle.getParam("/alpha", alpha)) {
    ROS_ERROR_STREAM("CartesianImpedanceCBFInteractionController: Could not read parameter alpha");
    return false;
  }

  if (!node_handle.getParam("/cbf_active", cbf_active)) {
    ROS_ERROR_STREAM("CartesianImpedanceCBFInteractionController: Could not read parameter cbf_active");
    return false;
  }

  if (!node_handle.getParam("/damping_ratio", damping_ratio)) {
    ROS_ERROR_STREAM("CartesianImpedanceCBFInteractionController: Could not read parameter damping_ratio");
    return false;
  }

  if (!node_handle.getParam("/Kmax", Kmax)) {
    ROS_ERROR_STREAM("CartesianImpedanceCBFInteractionController: Could not read parameter Kmax");
    return false;
  }
  ...
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "CartesianImpedanceCBFInteractionController: Invalid or no joint_names parameters provided, "
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceCBFInteractionController: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "CartesianImpedanceCBFInteractionController: Exception getting model handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceCBFInteractionController: Error getting state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "CartesianImpedanceCBFInteractionController: Exception getting state handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceCBFInteractionController: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "CartesianImpedanceCBFInteractionController: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  ...
  dynamic_server_compliance_param_->setCallback(
      boost::bind(&CartesianImpedanceCBFInteractionController::complianceParamCallback, this, _1, _2));
...
void CartesianImpedanceCBFInteractionController::starting(const ros::Time& /*time*/) {
...
void CartesianImpedanceCBFInteractionController::update(const ros::Time& time,
  ...
  // tau_d << tau_task + tau_nullspace + coriolis + gravity; // note that gravity is done by Franka itself, so has to be subtracted again after cbf computation
  ...
  std::tie(tau_cbf, h) = cbfCompute(tau_d, mass, gravity, Eigen::Map<Eigen::Matrix<double, 7, 1>>(dq_saturated.data()),tau_ext_flipped);
  ...
  // tau_cbf -= gravity; // remove gravity portion of control input again
...
Eigen::Matrix<double, 7, 1> CartesianImpedanceCBFInteractionController::saturateTorqueRate(
...
Eigen::Matrix<double, 7, 1> CartesianImpedanceCBFInteractionController::saturateQdotRate(
...
std::tuple<Eigen::Matrix<double, 7, 1>, double> CartesianImpedanceCBFInteractionController::cbfCompute(
    const Eigen::Matrix<double, 7, 1>& tau_d_calculated,
  ...
  // see by https://osqp.org/docs/examples/setup-and-solve.html
  ...
  upper_bounds << alpha*h - tau_ext.transpose()*dq, tau_max_;
  // for (int i = 0; i < 8; i++) {
  ...
  // ROS_INFO("solved problem");
  // TO DO: PRINT IF NOT ABLE TO SOLVE
...
std::tuple<Eigen::Matrix<double, 7, 1>, double> CartesianImpedanceCBFInteractionController::cbfAnalytical(
    const Eigen::Matrix<double, 7, 1>& tau_d_calculated,
    const Eigen::Map<Eigen::Matrix<double, 7, 7>>& D,
    const Eigen::Map<Eigen::Matrix<double, 7, 1>>& G,
    const Eigen::Map<Eigen::Matrix<double, 7, 1>>& dq) {
...
void CartesianImpedanceCBFInteractionController::complianceParamCallback(
...
void CartesianImpedanceCBFInteractionController::equilibriumPoseCallback(
  ...
  // double x = position_d_target_(0);
  // ROS_INFO("x: %f",x);
  // ROS_INFO(msg->pose.position.y);
  // ROS_INFO(msg->pose.position.z);
}

Eigen::Matrix<double, 7, 1> CartesianImpedanceCBFInteractionController::EMA(
...
PLUGINLIB_EXPORT_CLASS(franka_example_controllers::CartesianImpedanceCBFInteractionController,
