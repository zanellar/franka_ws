...
#include <franka_example_controllers/cartesian_impedance_dead_zone_cbf_controller.h>
...
bool CartesianImpedanceDZCBFController::init(hardware_interface::RobotHW* robot_hw,
                                               ros::NodeHandle& node_handle) {
  std::vector<double> cartesian_stiffness_vector;
  std::vector<double> cartesian_damping_vector;

  sub_equilibrium_pose_ = node_handle.subscribe(
      "/trajectory_publisher/equilibrium_pose", 20, &CartesianImpedanceDZCBFController::equilibriumPoseCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());

  std::string arm_id;
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR_STREAM("CartesianImpedanceDZCBFController: Could not read parameter arm_id");
    return false;
  }

  if (!node_handle.getParam("/alpha", alpha)) {
    ROS_ERROR_STREAM("CartesianImpedanceDZCBFController: Could not read parameter alpha");
    return false;
  }

  if (!node_handle.getParam("/cbf_active", cbf_active)) {
    ROS_ERROR_STREAM("CartesianImpedanceDZCBFController: Could not read parameter cbf_active");
    return false;
  }

  if (!node_handle.getParam("/damping_ratio", damping_ratio)) {
    ROS_ERROR_STREAM("CartesianImpedanceDZCBFController: Could not read parameter damping_ratio");
    return false;
  }

  if (!node_handle.getParam("/Kmax", Kmax)) {
    ROS_ERROR_STREAM("CartesianImpedanceDZCBFController: Could not read parameter Kmax");
    return false;
  }

  if (!node_handle.getParam("/deadzone", deadzone)) {
    ROS_ERROR_STREAM("CartesianImpedanceDZCBFController: Could not read parameter deadzone");
    return false;
  }

  if (!node_handle.getParam("/power", power)) {
    ROS_ERROR_STREAM("CartesianImpedanceDZCBFController: Could not read parameter power");
    return false;
  }
  ...
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "CartesianImpedanceDZCBFController: Invalid or no joint_names parameters provided, "
  ...
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceDZCBFController: Error getting model interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "CartesianImpedanceDZCBFController: Exception getting model handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceDZCBFController: Error getting state interface from hardware");
    return false;
  }
  try {
    ...
    ROS_ERROR_STREAM(
        "CartesianImpedanceDZCBFController: Exception getting state handle from interface: "
        << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "CartesianImpedanceDZCBFController: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM(
          "CartesianImpedanceDZCBFController: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  ...
  dynamic_server_compliance_param_->setCallback(
      boost::bind(&CartesianImpedanceDZCBFController::complianceParamCallback, this, _1, _2));
...
void CartesianImpedanceDZCBFController::starting(const ros::Time& time) {
...
void CartesianImpedanceDZCBFController::update(const ros::Time& time,
  ...
  bool action = error.head(3).norm() >= deadzone;
  if (action){
    error.head(3) << error.head(3)-error.head(3)/error.head(3).norm()*deadzone;
  }
  else {
  ...
  // "difference" quaternion (set to none here)
  Eigen::Quaterniond error_quaternion(orientation_d_.inverse() * orientation_d_);
  ...
  // compute control
  // allocate variables
  Eigen::VectorXd tau_task(7), tau_nullspace(7), tau_d(7), tau_cbf(7), tau_sat(7), tau_ext(7), W_ext(6);
  ...
  // Cartesian PD control with damping ratio = cartesian_damping
  tau_task << jacobian.transpose() *
                  (-cartesian_stiffness_ * error - action*cartesian_damping_ * (jacobian * dq));
  ...
  // apply "external power", in this case just unaccounted control input
  // wait 2 seconds before applying
  // NOTE: some hardcoded variables, very botched script, but it works. Not proud of it though...
  W_ext.setZero();
  if ((time-starttime).toSec() > 2){
    double v_y = (jacobian*dq)(1,0);
    double F_max = 50; // max force, N
    if (power > 0){
      if (v_y/power > 1/F_max){
        W_ext(1) = power/v_y;
      }
      else{
        W_ext(1) = F_max;
  ...
  tau_ext = jacobian.transpose()*W_ext;
  ...
  Eigen::Map<Eigen::VectorXd>(&cbf_info.u_ext[0], 7, 1) = tau_ext;
  cbf_info.header.stamp = time;
  cbf_publisher.publish(cbf_info);
}

Eigen::Matrix<double, 7, 1> CartesianImpedanceDZCBFController::saturateTorqueRate(
...
Eigen::Matrix<double, 7, 1> CartesianImpedanceDZCBFController::saturateQdotRate(
...
std::tuple<Eigen::Matrix<double, 7, 1>, double> CartesianImpedanceDZCBFController::cbfCompute(
    const Eigen::Matrix<double, 7, 1>& tau_d_calculated,
  ...
  // hp = (alpha*h+G.transpose()*dq).sparseView(); // NOTE: GRAVITY COMPENSATION IS DONE BY ROBOT ITSELF, SO TECHNICALLY NOT NEEDED
...
std::tuple<Eigen::Matrix<double, 7, 1>, double> CartesianImpedanceDZCBFController::cbfAnalytical(
    const Eigen::Matrix<double, 7, 1>& tau_d_calculated,
    const Eigen::Map<Eigen::Matrix<double, 7, 7>>& D,
    const Eigen::Map<Eigen::Matrix<double, 7, 1>>& G,
    const Eigen::Map<Eigen::Matrix<double, 7, 1>>& dq) {
...
void CartesianImpedanceDZCBFController::complianceParamCallback(
...
void CartesianImpedanceDZCBFController::equilibriumPoseCallback(
...
Eigen::Matrix<double, 7, 1> CartesianImpedanceDZCBFController::EMA(
...
PLUGINLIB_EXPORT_CLASS(franka_example_controllers::CartesianImpedanceDZCBFController,
