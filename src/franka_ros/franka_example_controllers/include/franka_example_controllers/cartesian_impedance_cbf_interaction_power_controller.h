...
class CartesianImpedanceCBFInteractionPowerController : public controller_interface::MultiInterfaceController<
 ...
 private:
  ...
  std::tuple<Eigen::Matrix<double, 7, 1>, double, double> cbfCompute(
    const Eigen::Matrix<double, 7, 1>& tau_d_calculated,
  ...
  Eigen::Matrix<double, 1, 1> hK;
  Eigen::Matrix<double, 1, 1> hP;  
  Eigen::SparseMatrix<double> hp{1, 1};
  Eigen::SparseMatrix<double> objective_matrix{7,7}; // P = I
  Eigen::Matrix<double, 1+1+7, 7> constraint_matrix;
  Eigen::Matrix<double, 1+1+7, 1> lower_bounds;
  Eigen::Matrix<double, 1+1+7, 1> upper_bounds;
