...
const double fstart = 0; // starting frequency [Hz]
const double fend = 1; // ending frequency [Hz]
const double amplitude = 0.4; // motion amplitude [m]
const double Tramp = 10; // time between start frequency and end frequency [s]
const double cramp = (fstart-fend)/Tramp;

geometry_msgs::PoseStamped trajectory(ros::Time starttime, ros::Time rostime) {
    double t = (rostime-starttime).toSec();
    // double T = abs(fmod(t-Tramp,2*Tramp)-Tramp);
    double T = 2*Tramp*abs(t/(2*Tramp)-floor(t/(2*Tramp)+.5));
    // ROS_INFO("ros time %f seconds", T);
    geometry_msgs::PoseStamped pose;
    pose.pose.position.x = 0.5;
    pose.pose.position.y = amplitude*sin(2*pi*(cramp/2*T*T+fstart*T));
