...
// chirp starting at frequency 0 (amplitude 30 cm) ending at frequency .5Hz
const double zstart = 0.6; // initial height
const double Cspring = 200; // spring constant [N/m]
const double Ftension = 50; // desired tension [N]
const double Tstart = 5; // seconds after start before tensioning
const double Tramp = 3; // seconds between no and full tension

const double offset = Ftension/Cspring;
...
geometry_msgs::PoseStamped trajectory(ros::Time starttime, ros::Time rostime) {
    ...
    if(t<Tstart){
      pose.pose.position.z = zstart;
    }
    else if(t<Tstart+Tramp){
      pose.pose.position.z = zstart + (offset/Tramp)*(t-Tstart);
    }
    else{
      pose.pose.position.z = zstart + offset;
    }
