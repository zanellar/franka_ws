...
const double freq = 0.2; // starting frequency [Hz]
const double amplitude = 0.2; // motion amplitude [m]
...
geometry_msgs::PoseStamped trajectory(ros::Time starttime, ros::Time rostime) {
    ...
    pose.pose.position.y = amplitude*(4*floor(freq*t)-2*floor(2*freq*t)+1);
